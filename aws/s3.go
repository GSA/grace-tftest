...
func S3BucketEncryptionRuleMatcher(keyArn string, algorithm string) func(*s3.ServerSideEncryptionRule) bool {
	return func(r *s3.ServerSideEncryptionRule) bool {
		if len(keyArn) > 0 && keyArn != *r.ApplyServerSideEncryptionByDefault.KMSMasterKeyID {
			return false
		}
		if len(algorithm) > 0 && algorithm != *r.ApplyServerSideEncryptionByDefault.SSEAlgorithm {
			return false
		}
		return true
	}
